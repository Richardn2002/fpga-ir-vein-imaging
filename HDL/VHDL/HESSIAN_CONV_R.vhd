...
ENTITY HESSIAN_conv_r IS
    PORT (
        ...
        clahe_in_addr : OUT STD_LOGIC_VECTOR(constants.HESSIAN_IN_ADDR_BITS - 1 DOWNTO 0);
        clahe_in_d : IN STD_LOGIC_VECTOR(7 DOWNTO 0);

        -- Output convolution RAM
        conv_out_en : OUT STD_LOGIC;
        conv_out_addr : OUT STD_LOGIC_VECTOR(constants.HESSIAN_OUT_ADDR_BITS - 1 DOWNTO 0);
...
ARCHITECTURE rtl OF HESSIAN_conv_r IS
    ...
    CONSTANT IN_X : INTEGER := 96; -- 96
    CONSTANT IN_Y : INTEGER := 96;
    CONSTANT OUT_X : INTEGER := 90; --shaves off left and right 3 pixels
    CONSTANT OUT_Y : INTEGER := 90; 
    CONSTANT RADIUS : INTEGER := 3; -- = 3

    TYPE state_t IS (
        IDLE,
        ROW_PRIME_REQ, ROW_PRIME_WAIT, ROW_PRIME_SHIFT,
        PIX_REQ, PIX_WAIT,
        COMPUTE,
        WRITE_OUT,
        NEXT_PIXEL,
    ...
    -- column in input space = x + RADIUS
    SIGNAL in_col : INTEGER;
    ...
    SIGNAL sum_next : signed(15 DOWNTO 0);
    ...
    BEGIN
        ...
        CASE state IS
            ...
            WHEN ROW_PRIME_REQ =>
                ...
                clahe_in_en <= '1';
                clahe_in_addr <= STD_LOGIC_VECTOR(to_unsigned(in_addr, clahe_in_addr'length));
                state_next <= ROW_PRIME_WAIT;
            ...
            WHEN WRITE_OUT =>
                ...
                conv_out_en <= '1';
                conv_out_addr <= STD_LOGIC_VECTOR(to_unsigned(out_addr, conv_out_addr'length));
