...
ENTITY HESSIAN_conv_r IS
    PORT (
        ...
        clahe_in_addr : OUT STD_LOGIC_VECTOR(14 - 1 DOWNTO 0);
        clahe_in_d    : IN  STD_LOGIC_VECTOR(7 DOWNTO 0);

        -- Output convolution RAM
        conv_out_en   : OUT STD_LOGIC;
        conv_out_addr : OUT STD_LOGIC_VECTOR(14 - 1 DOWNTO 0);
...
ARCHITECTURE rtl OF HESSIAN_conv_r IS
    ...
    CONSTANT OUT_X   : INTEGER := 90;  -- shave off 3 pixels each side
    CONSTANT OUT_Y   : INTEGER := 90;
    CONSTANT RADIUS  : INTEGER := 3;

    TYPE state_t IS (
        IDLE,
        ROW_PRIME_REQ, ROW_PRIME_WAIT,
        PIX_REQ, PIX_WAIT,
        COMPUTE,
        WRITE_OUT,
        NEXT_PIXEL,
...
BEGIN
    ...
    BEGIN
        ...
        CASE state IS
            ...
            WHEN PIX_REQ =>
                in_addr := (y + RADIUS) * IN_X + (x + (RADIUS + 3));
                clahe_in_en   <= '1';
                clahe_in_addr <= STD_LOGIC_VECTOR(to_unsigned(in_addr, clahe_in_addr'length));
                state_next    <= PIX_WAIT;
